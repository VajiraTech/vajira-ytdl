...
let oldCookieWarning = false;
let oldDispatcherWarning = false;
...
exports.getBasicInfo = async(id, options) => {
  ...
  let { jar, dispatcher } = options.agent || cookie.defaultAgent;
  if (!options.agent) {
    const c = utils.getPropInsensitive(options.requestOptions.headers, 'cookie');
    if (c) {
      jar.removeAllCookiesSync();
      cookie.addCookiesFromString(jar, c);
      if (!oldCookieWarning) {
        oldCookieWarning = true;
        console.warn(
    ...
    if (options.requestOptions.dispatcher && !oldDispatcherWarning) {
      ...
      console.warn(
        ...
        'To implement your own, check out the documentation. (https://github.com/distubejs/ytdl-core#usage)',
      );
    }
