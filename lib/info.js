...
const getWatchHTMLPage = async(id, options) => {
  ...
  try {
    ...
    try {
      ...
      const filename = `watch-${+new Date()}.html`;
      require('fs').writeFileSync(filename, body);
      throw Error(
        'Error parsing watch.html, maybe YouTube made a change.\n' +
        `Please report this issue with the contents of "${filename}" file on @distubejs/ytdl GitHub.`,
      );
    }
  }
