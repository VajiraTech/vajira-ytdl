...
exports.extractFunctions = body => {
  ...
  const extractDecipher = () => {
    try {
      ...
      const deobfuscationFunctionName = getDeobfuscationFuncName(body).replaceAll('$', '\\$');
      const functionPattern = `(${deobfuscationFunctionName}=function\\([a-zA-Z0-9_]+\\)\\{.+?\\})`;
      ...
      const callerFunction = `${deobfuscationFunctionName}(sig);`;
