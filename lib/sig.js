...
const getFuncName = (body, regexps) => {
  ...
  if (fn.includes('[')) {
    throw Error('YouTube changed their response, please open an issue on @distube/ytdl-core GitHub.');
  }
  return fn;
};
...
exports.extractFunctions = body => {
  ...
  const extractDecipher = () => {
    ...
    try {
      ...
      const helperObjectName = matchGroup1(';([A-Za-z0-9_\\$]{2,})\\.\\w+\\(', decipherFunc).replaceAll('$', '\\$');
      ...
      const resultFunc = helperObject + decipherFunc + callerFunc;
      functions.push(resultFunc);
    } catch (err) {
  ...
  const extractNTransform = () => {
    try {
      ...
      if (nFuncName && nFuncName.length) {
        ...
        const resultFunc = nTransformFunc + callerFunc;
        functions.push(resultFunc);
      } else {
        throw new Error();
      }
